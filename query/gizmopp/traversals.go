...
func (p *pathObject) Regex(call goja.FunctionCall, vm *goja.Runtime) goja.Value {
...
func (p *pathObject) Like(call goja.FunctionCall, vm *goja.Runtime) goja.Value {
	args := exportArgs(call.Arguments)
	if len(args) != 1 {
...
func (p *pathObject) Compare(call goja.FunctionCall, vm *goja.Runtime) goja.Value {
	args := exportArgs(call.Arguments)
	if len(args) != 2 {
	...
	op, ok := toInt(args[0])
	if !ok {
		panic(vm.ToValue(errType{Expected: 1, Got: op}))
	}
